...
#cut
cgc.ffm$stockcut <- cut(cgc.ffm$stocklog, breaks=7, labels=FALSE,
                        ordered_result = TRUE)
plot(cgc.ffm$stockcut)
head(cgc.ffm)
#cgc.ffm[c(1000,500,25,36,10000),]
cgc.ffm2 <- cgc.ffm[1:4000,]
ggplot() +
  ...
  geom_line(aes(long, lat, group=group),
            alpha = 0.2, data=cgc.ffm2, col="skyblue2", size=0.3) + #drawing great circle lines
